...
graph TD
    ...
    Catalog -->|Read Cache| Redis["GCP Memorystore for Redis Cluster"]
...
Our microservices are built in **Spring Boot** and deployed on **Google Kubernetes Engine (GKE)**:
...
### 1.3. Multi-Tier Distributed Caching
To prevent database bottlenecks under heavy write/read traffic:
*   **Caching Strategy (Cache-Aside):** Read-heavy APIs query **GCP Memorystore for Redis Cluster** first. If a cache miss occurs, the backend reads from the database and populates Redis with an explicit TTL (Time-to-Live).
*   **Redis Cluster:** High-availability configuration with automatic failover, replication, and horizontal sharding, ensuring sub-millisecond response times.
